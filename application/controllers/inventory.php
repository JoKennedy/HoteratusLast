...
class Inventory extends Front_Controller
{
	...
	{
		$date = urldecode($date);
//		$errors = get_data('channel_error', array(
//            'user_id' => current_user_type(),
//            'hotel_id' => hotel_id(),
//            'error_occurs IN' => "(Bulk Update Save,Bulk Update)",
//            'timestamp  >=' => $date
//        ));

		$this->db->select('*');
		$this->db->where("(error_occurs='Bulk Update Save' OR error_occurs='Bulk Update')", NULL, FALSE);
		$this->db->where('user_id', current_user_type());
		$this->db->where('hotel_id', hotel_id());
		$this->db->where('timestamp >=', $date);
		$query = $this->db->get('channel_error');
		echo $this->db->last_query();
		$errors = $query->result_array();        
		$table = '<link href="'.base_url().'user_assets/css/bootstrap.min.css" type="text/css" rel="stylesheet" /><table class="table table-striped table-bordered table-hover" width="100%;height:100%">';
		...
		$table .= '<th>Price</th>';
		$table .= '<th>Minimum Stay</th>';
		$table .= '<th>CTA</th>';
		$table .= '<th>CTD</th>';
		$table .= '<th>Stop_sell</th>';
		$table .= '<th>Open Room</th>';
		$table .= '<th>Message</th>';
		...
		if (count($errors) > 0) {
			foreach ($errors as $error) {
				$channel = get_data('manage_channel', array(
					'channel_id' => $error['channel_id']
				))->row();
				
				$table .= '<tr class="info">';
				$table .= '<td><img src="data:image/png;base64,'. base64_encode(file_get_contents("uploads/".$channel->image."")).'" style="height: 35px;width: 85px;">'.$channel->channel_name.'</td>';
				$table .= '<td>'.$error['start_date'].'</td>';
				$table .= '<td>'.$error['end_date'].'</td>';
				$table .= '<td>'.$error['availability'].'</td>';
				$table .= '<td>'.$error['price'].'</td>';
				$table .= '<td>'.$error['minimum_stay'].'</td>';
				$table .= '<td>'.$error['cta'].'</td>';
				$table .= '<td>'.$error['ctd'].'</td>';
				$table .= '<td>'.$error['stop_sell'].'</td>';
				$table .= '<td>'.$error['open_room'].'</td>';
				if (strlen($error['error_message']) > 100) {
					$table .= '<td><textarea style="height: 100px;overflow-y:scroll;">'.$error['error_message'].'</textarea></td>';
				} else {
					$table .= '<td>'.$error['error_message'].'</td>';
				}

				$table .= '<td>'.$error['error_date_time'].'</td>';
				$table .= '</tr>';
			}
		}else{
			$table .= '<tr class="info">';
			$table .= '<td colspan="12" align="center">No errors Found !!</td>';
			$table .= '</tr>';
		}
		$table .= '</table>';
		echo $table;
    ...
    function saveImagesOrders(){
        $udata = array();
        $udata['photo_names'] = preg_replace('/,$/is','',$this->input->post('image_det'));

        // print "image_det=>".$this->input->post('image_det');
        // print "hotelDte=>".$this->input->post('hotelid');

        if(update_data(TBL_PHOTO,$udata,array('room_id'=>$this->input->post('hotelid'))))
        {
            return true;
        }else{
            return false;
    ...
    function get_channel_room($pro_id)
    {
        ...
        $get_channel_id = explode(',', get_data(TBL_PROPERTY, array(
            'property_id' => $pro_id
        ))->row()->connected_channel);
        $get_channel    = get_data(TBL_CHANNEL, array(
            'status' => 'Active'
        ))->result_array();
...
?>
     <div class="checkbox">
         <label>    <input type="checkbox" checked="checked" class="channel_single" name="channel_single[]" value="<?php
                    echo $channel_id;
?>"> <?php
                    echo $channel_name;
?>  </label>
...
?>
    ...
    function single_update($mode = '', $id = '')
    {
        ...
        switch ($mode) {
            ...
            case 'update':
                extract($this->input->post());
                $available = get_data(TBL_UPDATE, array(
                    'room_id' => ($up_property)
                ))->row_array();
                if (count($available) == 0) {
                    ...
                    if (update_data(TBL_UPDATE, $idata, array(
                        'room_id' => ($up_property)
                    ))) {
                        redirect('inventory/single_update', 'refresh');
                    }
    ...
    function bulk_update($mode = '', $id = '', $resdata = array())
    {
        ...
        switch ($mode) {
                        ...
                        foreach ($array as $id => $room) {
                            ...
                            if(isset($room['price'])!=0 && isset($room['price']) !='')
                            {
                                ...
                                $this->db->query("Update  room_update                                  
                                    ...
                                    and STR_TO_DATE(separate_date ,'%d/%m/%Y')   between STR_TO_DATE('".$room['start_date']."' ,'%d/%m/%Y')  and STR_TO_DATE('".$room['end_date'] ."' ,'%d/%m/%Y') " );
                            }
                            ...
                            if(isset($room['availability']) && !isset($room['price']))
                            {
                                $revenueprice=  $this->db->query("select  case when ((((percentage/100)*PriceRevenue)/existing_room_count)*(existing_room_count-".$room['availability'].")) + PriceRevenue > maximun then maximun else ((((percentage/100)*PriceRevenue)/existing_room_count)*(existing_room_count-".$room['availability'].")) + PriceRevenue end precio
                                        ...
                                        and STR_TO_DATE(b.separate_date ,'%d/%m/%Y')   between STR_TO_DATE('".$room['start_date']."' ,'%d/%m/%Y')  and STR_TO_DATE('".$room['end_date']."' ,'%d/%m/%Y') " )->row()->precio;


                                if ($revenueprice!="")
                                {
                                    //$room['price']=$revenueprice;
                                }
                            }
                            ...
                            foreach ($postArr as $key => $value) {
                                if (preg_match('/start_date(\d+)/is', $key, $match)) {
                                    $room['start_date'] = $this->input->post('start_date' . $match[1]);
                                    $room['end_date']   = $this->input->post('end_date' . $match[1]);
                                    if ($room['start_date'] != '' and $room['end_date'] != '') {
                                          if(isset($room['price'])!=0 && isset($room['price']) !='')
                                            {
                                        ...
                                        $this->inventory_model->save($room, '', '', 'res_update');
                                    }
                                }
                            }
                        }
                    ...
                    if (is_array($arrayrate) && count($arrayrate) != 0) {
                        ...
                        foreach ($arrayrate as $id => $room) {
                            ...
                            foreach ($room as $rateid => $rate) {
                                  ...
                                  if(isset($rate['price'])!=0 && isset($rate['price']) !='')
                                    {
                                        ...
                                        $this->db->query("Update  room_rate_types_base                                  
                                            ...
                                            where hotel_id=".hotel_id()."
                                            and room_id=".$rate['room_id']."
                                            and rate_types_id=".$rate['rate_id']."
                                            and individual_channel_id=0
                                            and STR_TO_DATE(separate_date ,'%d/%m/%Y')   between STR_TO_DATE('".$rate['start_date']."' ,'%d/%m/%Y')  and STR_TO_DATE('".$rate['end_date'] ."' ,'%d/%m/%Y') " );
                                    }
                                    ...
                                    if(isset($rate['availability']) && !isset($rate['price']))
                                    {
                                        $revenueprice=  $this->db->query("select  case when ((((percentage/100)*PriceRevenue)/existing_room_count)*(existing_room_count-".$rate['availability'].")) + PriceRevenue > maximun then maximun else ((((percentage/100)*PriceRevenue)/existing_room_count)*(existing_room_count-".$rate['availability'].")) + PriceRevenue end precio
                                                ...
                                                and b.hotel_id=".hotel_id()."   
                                                AND a.revenuertatus =1
                                                and b.rate_types_id=".$rate['rate_id']." 
                                                and a.property_id=".$rate['room_id']."
                                                and b.individual_channel_id=0
                                                and STR_TO_DATE(b.separate_date ,'%d/%m/%Y')   between STR_TO_DATE('".$rate['start_date']."' ,'%d/%m/%Y')  and STR_TO_DATE('".$rate['end_date']."' ,'%d/%m/%Y') " )->row()->precio;


                                        if ($revenueprice!="")
                                        {
                                            //$rate['price']=$revenueprice;
                                        }
                                    }
                                ...
                                foreach ($postArr as $key => $value) {
                                    if (preg_match('/start_date(\d+)/is', $key, $match)) {
                                        $rate['start_date'] = $this->input->post('start_date' . $match[1]);
                                        $rate['end_date']   = $this->input->post('end_date' . $match[1]);
                                        if ($room['start_date'] != '' and $room['end_date'] != '') {
                                             if(isset($rate['price'])!=0 && isset($rate['price']) !='')
                                                {
            ...
            case 'res_update':
                if (user_type() == '1' || user_type() == '2' && in_array('3', user_edit()) || admin_id() != '' && admin_type() == '1') {
                    ...
                    if (is_array($array) && count($array) != 0) {
                        ...
                        foreach ($array as $id => $room) {
                            foreach ($room as $key => $value) {
                                if (!in_array($key, $resdata['updatevalue'])) {
                                    unset($room[$key]);
                                }
                            }
                            
                            $room['room_id']    = $id;
                            $room['start_date'] = $resdata['start_date'];
                            $room['end_date']   = $resdata['end_date'];
                            ...
                            if ($resdata['days'] != '') {
                                $room['days'] = implode(',', $resdata['days']);
                            } else {
                                $room['days'] = ('1,2,3,4,5,6,7');
                            }
                            
                            
                            if ($resdata['channel_id'] != "") {
                                $room['channel_id'] = implode(',', $resdata['channel_id']);
                            }
                            
                            //$this->inventory_model->save($room);
                            $this->inventory_model->save($room, '', '', $resdata);
                            
                            $postArr = $resdata;
                            // print "room=>";print_r($postArr);
                            // print "room=>";print_r($room);
                            foreach ($postArr as $key => $value) {
                                if (preg_match('/start_date(\d+)/is', $key, $match)) {
                                    $room['start_date'] = $this->input->post('start_date' . $match[1]);
                                    $room['end_date']   = $this->input->post('end_date' . $match[1]);
                                    if ($room['start_date'] != '' and $room['end_date'] != '') {
                                        //print "room1=>";print_r($room);
                                        $this->inventory_model->save($room, '', '', $resdata);
                                    }
                                }
                            }
            ...
            default:
                if (user_type() == '1' || user_type() == '2' && in_array('3', user_edit()) || admin_id() != '' && admin_type() == '1') {
                    ...
                    $data['bulk_room']    = get_data(TBL_PROPERTY, array(
                        'status' => 'Active',
                        'owner_id' => current_user_type(),
                        'hotel_id' => hotel_id()
                    ))->result_array();
                    ...
                    $data['hotel_detail'] = get_data(HOTEL, array(
                        'owner_id' => current_user_type(),
                        'hotel_id' => hotel_id()
                    ), 'currency')->row_array();
    ...
    function saveuserconfig()
    {
        ...
        $available1            = get_data('ConfigUsers', array(
            'userid' => current_user_type(),
            'Hotelid' => hotel_id()
        ))->row_array();
            ...
            switch ($mode) {
                case 'view':
                    if (user_type() == '1' || admin_id() != '' && admin_type() == '1') {
                        $available = get_data(TBL_PROPERTY, array(
                            'property_id' => (insep_decode($id)),
                            'owner_id' => user_id(),
                            'hotel_id' => hotel_id()
                        ))->row_array();
                    } else if (user_type() == '2') {
                    ...
                    if (count($available) != 0) {
                        ...
                        if (user_type() == '1' || admin_id() != '' && admin_type() == '1') {
                            $data['active'] = get_data(TBL_PROPERTY, array(
                                'owner_id' => user_id(),
                                'hotel_id' => hotel_id(),
                                'status' => 'Active'
                            ))->result_array();
    ...
    function update_amenities()
    {
        ...
        if (user_type() == '1' || admin_id() != '' && admin_type() == '1') {
            $room_amenities = update_data(TBL_PROPERTY, $udata, array(
                'owner_id' => user_id(),
                'hotel_id' => hotel_id(),
    ...
    function rate_management()
    {
        ...
        if ($this->input->post('ical_plan')) {
            ...
            if ($planid != '') {
                ...
                $currency_tbl = get_data(TBL_CUR, array(
                    'currency_id' => $subscribe_plan['currency']
                ))->row_array();
                ...
                $data['card_count']           = $this->db->select('id ')->from(TBL_CREDIT)->where(array(
                    'user_id' => user_id()
                ))->count_all_results();
                $data['subscribe_channel_id'] = 0;
                if ($data['card_count'] != 0) {
                    $data['cards'] = get_data(TBL_CREDIT, array(
                        'user_id' => user_id()
                    ))->result_array();
                }
    ...
    function Buy_Now($payment_id = '')
    {
        ...
        if ($subscribe_plan_id != '' && $subscribe_channel_id != '') {
            ...
            $avilable = get_data(BILL, array(
                "user_id" => current_user_type(),
                "hotel_id" => hotel_id()
            ))->row_array();
            if (count($avilable) != 0) {
                update_data(BILL, $rdata, array(
                    'user_id' => current_user_type()
                ));
            } else {
            ...
            if ($this->input->post('free_subscribe') != 'Free') {
                $channels   = get_data(TBL_CHANNEL, array(
                    'status' => 'Active'
                ), 'channel_id')->result_array();
                ...
                if ($this->input->post('pay_paypal') ==1) {
                    $this->load->library('paypal_class');
                    
                    $admin_setting = get_data(ADMIN,array(
                        'id' => 1
                    ))->row();
                    
                    $payment_details = get_data(TBL_PLAN, array(
                        'plan_id' => insep_decode($subscribe_plan_id)
                    ))->row();
                    ...
                    <?php echo' <form action="'.$paypal_url.'" method="post" name="frmPayPal1">  <input type="hidden" name="business" value="'.$admin_setting->paypal_emailid.'">
                        <input type="hidden" name="cmd" value="_xclick"> <input type="hidden" name="rm" value="2"> <input type="hidden" name="item_name" value="Buy'.$payment_details->plan_name.' Package"> <input type="hidden" name="item_number" value="1">
                        <input type="hidden" name="amount" value="'.$payment_details->plan_price.'">
                    <input type="hidden" name="no_shipping" value="1">
                    <input type="hidden" name="currency_code" value="USD">
                    <input type="hidden" name="handling" value="0">
                    <input type="hidden" name="cancel_return" value="'.lang_url(). 'user/Payment_Cancel">
                    <input type="hidden" name="return" value="'.lang_url() .'inventory/Payment_Success/'.$payment_id.'">
                    <input type="hidden" name="custom" value="'.$connect_channels.'">
                    ...
                    if ($this->input->post('use_existing_card') == 'true') {
                        ...
                        $card_details = get_data(TBL_CREDIT, array(
                            'id' => $existing_card
                        ))->row();
                        ...
                        $country_c = get_data(TBL_COUNTRY, array(
                            'id' => $user_details->country
                        ))->row()->country_name;
            ...
            elseif ($free_subscribe == 'Free') {
                ...
                $plan_details  = get_data(TBL_PLAN, array(
                    'plan_id' => insep_decode($payment_id)
                ))->row();
    ...
    function Payment_Success($payment_id = '')
    {
        ...
        if ($payment_id != '') {
            if ($_REQUEST['txn_id'] != '') {
                ...
                $data['user_id']   = current_user_type();
                $data['hotel_id']  = hotel_id();
                $check_transaction = get_data(MEMBERSHIP, array(
                    'user_id' => current_user_type(),
                    'hotel_id' => hotel_id(),
    ...
    function update_more_options()
    {
        ...
        $mdata['area']                = $area;
        if (user_type() == '1' || admin_id() != '' && admin_type() == '1') {
            $update_data = update_data(TBL_PROPERTY, $mdata, array(
                'hotel_id' => hotel_id(),
                'owner_id' => user_id(),
    ...
    function update_basic_rooms()
    {
        ...
        if (user_type() == '1' || admin_id() != '' && admin_type() == '1') {
            $adult_count = get_data(TBL_PROPERTY, array(
                'hotel_id' => hotel_id(),
                'owner_id' => user_id(),
                'property_id' => insep_decode($property_id)
            ))->row()->member_count;
            if (update_data(TBL_PROPERTY, $bdata, array(
                'property_id' => insep_decode($property_id),
                'hotel_id' => hotel_id(),
                'owner_id' => user_id()
            ))) {
    ...
    function master_rate_update()
    {
        ...
        if (user_type() == '1' || admin_id() != '' && admin_type() == '1') {
            update_data(TBL_PROPERTY, $bdata, array(
                'property_id' => insep_decode($property_id),
                'hotel_id' => hotel_id(),
                'owner_id' => user_id()
            ));
            update_data(RATE_TYPES, $rdata, array(
                'room_id' => insep_decode($property_id),
                'hotel_id' => hotel_id(),
                'user_id' => user_id()
            ));
        } else if (user_type() == '2') {
        ...
        if (user_type() == '1' || admin_id() != '' && admin_type() == '1') {
            $add_room = get_data(TBL_PROPERTY, array(
                'property_id' => insep_decode($property_id),
                'hotel_id' => hotel_id(),
                'owner_id' => user_id()
            ))->row();
        ...
        switch ($mode) {
            case 'get':
                ...
                $available = get_data(TBL_PROPERTY, array(
                    'owner_id' => current_user_type(),
                    'hotel_id' => hotel_id(),
                    'property_id' => insep_decode($hotel_id)
                ))->row_array();
                /*}
                else if(user_type()=='2'){
                ...
                if (count($available) != 0) {
                    $datas['hotel_photos'] = get_data(TBL_PHOTO, array(
                        'user_id' => current_user_type(),
                        'hotel_id' => hotel_id(),
                        'room_id' => insep_decode($hotel_id)
                    ))->row_array();
            ...
            case 'new':
                ...
                //print "files=>";print_r($files);
                foreach ($files as $filename)
                    $result = $this->inventory_model->add_photo($filename);
                if ($result) {
                    // if(user_type()=='1'){
                    $data['hotel_photos'] = get_data(TBL_PHOTO, array(
                        'user_id' => current_user_type(),
                        'hotel_id' => hotel_id(),
                        'room_id' => insep_decode($hotel_id)
                    ))->row_array();
            ...
            case 'remove':
                // if(user_type()=='1'){
                $value        = get_data(TBL_PHOTO, array(
                    'user_id' => current_user_type(),
                    'hotel_id' => hotel_id(),
                    'photo_id' => insep_decode($photo_id)
                ))->row()->photo_names;
                /*}
                else if(user_type()=='2'){
                ...
                // if(user_type()=='1'){
                if (update_data(TBL_PHOTO, $updata, array(
                    'user_id' => current_user_type(),
                    'hotel_id' => hotel_id(),
                    'photo_id' => insep_decode($photo_id)
                ))) {
        ...
        switch ($mode) {
            case 'add':
                ...
                if (insert_data(RATE_TYPES, $idata)) {
                    ...
                    $add_room = get_data(RATE_TYPES, array(
                        'hotel_id' => hotel_id(),
                        'user_id' => current_user_type(),
                        'uniq_id' => $u_id
                    ))->row();
                ...
                break;
            case "edit":
                $available = get_data(RATE_TYPES, array(
                    'hotel_id' => hotel_id(),
                    'user_id' => current_user_type(),
                    'uniq_id' => (($id))
                ))->row_array();
                ...
                if (count($available) != 0) {
                    $data                 = array_merge($data, $available);
                    $data['types_refund'] = get_data(RATE_TYPES_REFUN, array(
                        'hotel_id' => hotel_id(),
                        'user_id' => current_user_type(),
                        'uniq_id' => $id
                    ))->result_array();
            ...
            case 'update':
                ...
                if (user_type() == '1' || admin_id() != '' && admin_type() == '1') {
                    $room_rate = update_data(RATE_TYPES, $idata, array(
                        'hotel_id' => hotel_id(),
                        'user_id' => current_user_type(),
                        'uniq_id' => $uniq_id
                    ));
                ...
                if ($room_rate) {
                    ...
                    if ($add_room->pricing_type == 2) {
                        for ($i = 1; $i <= $member_count; $i++) {
                            ...
                            update_data(RATE_TYPES_REFUN, $ndata, array(
                                'hotel_id' => hotel_id(),
                                'user_id' => current_user_type(),
                                'rate_type_id' => $this->input->post('rate_type_id_' . $i)
                            ));
                        }
                    } else if ($add_room->pricing_type == 1) /*&& $add_room->non_refund==1)*/ {
                        $i                     = 1;
            ...
            case 'delete':
                extract($this->input->post());
                $available = get_data(RATE_TYPES, array(
                    'uniq_id' => insep_decode($unq_id),
                    'room_id' => insep_decode($room_id),
                    'user_id' => current_user_type(),
                    'hotel_id' => hotel_id()
                ))->row_array();
                ...
                if (count($available) != 0) {
                    if (user_type() == '1' || admin_id() != '' && admin_type() == '1') {
                        $map_exists = get_data(MAP, array(
                            'property_id' => insep_decode($room_id),
                            'rate_id' => $available['rate_type_id'],
                            'owner_id' => current_user_type(),
                            'hotel_id' => hotel_id()
                        ))->num_rows();
                        if ($map_exists == 0) {
                            delete_data(RATE_TYPES, array(
                                'uniq_id' => insep_decode($unq_id),
                                'room_id' => insep_decode($room_id),
                                'user_id' => current_user_type(),
                                'hotel_id' => hotel_id()
                            ));
                            delete_data(RATE_TYPES_REFUN, array(
                                'uniq_id' => insep_decode($unq_id),
                                'room_id' => insep_decode($room_id),
                                'user_id' => current_user_type(),
                                'hotel_id' => hotel_id()
    ...
    function advance_update($mode = '', $id = '')
    {
        ...
        if ($mode === 'reservation_no') {
            ...
            switch ($mode) {
                case "reservation_no":
                    if (user_type() == '1' || user_type() == '2' && in_array('3', user_view()) || admin_id() != '' && admin_type() == '1') {
                        ...
                        $data                 = array_merge($data, $user_details);
                        $this->db->order_by('property_id', 'desc');
                        $data['all_room']    = get_data(TBL_PROPERTY, array(
                            'status' => 'Active',
                            'owner_id' => current_user_type(),
                            'hotel_id' => hotel_id(),
                            'droc' => '1'
                        ), 'property_id,non_refund,member_count,property_name,pricing_type')->result_array();
            ...
            $this->views('channel/inventory/advance_update_reservetion', array_merge(get_data(TBL_USERS, array(
                'user_id' => user_id()
            ))->row_array(), array(
    ...
    function customize_calender_new()
    {
        if (!IS_AJAX) {
            ...
            if (user_type() == '1' || user_type() == '2' && in_array('3', user_view()) || admin_id() != '' && admin_type() == '1') {
                ...
                $this->load->view('channel/inventory/customize_calender', $this->inventory_advance_update_model->getCalendarData());
            } else {
                echo '0';
            }
        }
    ...
    function inline_edit()
    {
        ...
        if (update_data(TBL_UPDATE, $udata, array(
            'owner_id' => current_user_type(),
            'hotel_id' => hotel_id(),
            'room_id' => $room_id,
    ...
    function inline_edit_no($data = array())
    {
        ...
        if (user_type() == '1' || user_type() == '2' && in_array('3', user_edit()) || admin_id() != '' && admin_type() == '1') {
            ...
            if (count($data) > 0) {
                ...
                $room_id = substr($data['pk'], strpos($data['pk'], "-") + 1);
                
                $date = explode("-", $data['pk']);
            ...
            $all_channel = get_data(CONNECT, array(
                'user_id' => current_user_type(),
                'hotel_id' => hotel_id(),
                'status' => 'enabled'
            ), 'channel_id')->result_array();
            ...
            if (count($all_channel != 0)) {
                foreach ($all_channel as $con_ch) {
                    ...
                    if (($con_ch == 2 && ($chk_allow == 2 || $chk_allow == 3)) || $con_ch != 2) {
                        $available1 = get_data(TBL_UPDATE, array(
                            'owner_id' => current_user_type(),
                            'hotel_id' => hotel_id(),
                            'individual_channel_id' => $con_ch,
                            'room_id' => $room_id,
                            'separate_date' => $update_date
                        ))->row_array();
                        if ($name == "price") {
                        ...
                        if ($name == 'price' || $name == 'availability') {
                            ...
                            $room_details = get_data(TBL_PROPERTY, array(
                                'owner_id' => current_user_type(),
                                'hotel_id' => hotel_id(),
                                'property_id' => $room_id
                            ))->row();
                            //print_r($room_details);
                            if ($room_details->meal_plan == '1' || $room_details->meal_plan == '0') {
                                $meal_plan = 0;
                            } elseif ($room_details->meal_plan == '2') {
                                $meal_plan = 1;
                            } elseif ($room_details->meal_plan == '3') {
                                $meal_plan = 3;
                            } elseif ($room_details->meal_plan == '4') {
                                $meal_plan = 0;
                            } elseif ($room_details->meal_plan == '5' || $room_details->meal_plan == '6') {
                                $meal_plan = 2;
                            }
                            
                            if ($name == 'price') {
                                $count = $this->db->select('mapping_id')->from(MAP)->where(array(
                                    'owner_id' => current_user_type(),
                                    'hotel_id' => hotel_id(),
                                    'channel_id' => $con_ch,
                                    'property_id' => $room_id,
                            ...
                            if ($count != 0) {
                                ...
                                if ($room_mapping) {
                                    foreach ($room_mapping as $room_value) {
                                        ...
                                        $update_Details = get_data(TBL_UPDATE, array(
                                            'individual_channel_id' => $con_ch,
                                            'room_id' => $room_id,
                                            'separate_date' => $update_date
                                        ))->row();
                                        ...
                                        if ($room_value->channel_id == '11') {
                                            ...
                                            $mp_details = get_data(IM_RECO, array(
                                                'user_id' => current_user_type(),
                                                'hotel_id' => hotel_id(),
                                                'channel_id' => $room_value->channel_id,
                                                're_id' => $room_value->import_mapping_id,
                                            ...
                                            if ($name == 'price' && $room_value->update_rate == '1') {
                                                $mapping_values = get_data('mapping_values', array(
                                                    'mapping_id' => $room_value->mapping_id
                                                ))->row_array();
                                                if ($mapping_values) {
                                            ...
                                            <SingleOcc>' . $update_Details->price . '</SingleOcc>' . $mapping_fields . '
                                            ...
                                            $mp_details = get_data('import_mapping', array(
                                                'user_id' => current_user_type(),
                                                'hotel_id' => hotel_id(),
                                                'channel' => $room_value->channel_id,
                                                'map_id' => $room_value->import_mapping_id
                                            ))->row();
                                            ...
                                            $mp_details = get_data('import_mapping_HOTELBEDS_ROOMS', array(
                                                'user_id' => current_user_type(),
                                                'hotel_id' => hotel_id(),
                                                'channel_id' => $room_value->channel_id,
                                                'map_id' => $room_value->import_mapping_id
                                            ...
                                            if ($name == 'availability' && $room_value->update_availability == '1') {
                                                $gt_details = get_data(CONNECT, array(
                                                    'user_id' => current_user_type(),
                                                    'hotel_id' => hotel_id(),
                                                    'channel_id' => $con_ch
                                                ))->row();
                                                ...
                                                $mp_details    = get_data('import_mapping_GTA', array(
                                                    'user_id' => current_user_type(),
                                                    'hotel_id' => hotel_id(),
                                                    'GTA_id' => $room_value->import_mapping_id,
                                                    'channel_id' => $room_value->channel_id
                                                ))->row();
                                            ...
                                            Quantity="' . $availability . '" ReleaseDays="0"/>
                                                ...
                                                $response = curl_exec($ch);
                                                makeLog('gtaUpdateLog.txt', "INLINE_EDIT_NO Alot URL=>$soapUrl\nContent=>$xml_post_string\nResponse=>$response\n");
                                            ...
                                            if ($name == 'price' && $room_value->update_rate == '1') {
                                                ...
                                                if ($contract_type == "Static") {
                                                   ...
                                                   <User Qualifier="' . $gt_details->web_id . '" UserName="' . $gt_details->user_name . '" Password="' . $gt_details->user_password . '" />
                                                    ...
                                                    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                                                    $output = curl_exec($ch);
                                                    makeLog('gtaUpdateLog.txt', "INLINE_EDIT_NO PRICE URL=>$soapUrl\nContent=>$xml_post_string\nResponse=>$output\n");
                                                    curl_close($ch);
                                                    $data        = simplexml_load_string($output);
                                                    //print_r($data);
                                                    $Error_Array = @$data->Errors->Error;
                                        ...
                                        } elseif ($room_value->channel_id == '9') {
                                            $channle_id     = 9;
                                            $airbnb_details = get_data(CONNECT, array(
                                                'user_id' => current_user_type(),
                                                'hotel_id' => hotel_id(),
                                            ...
                                            $mp_details     = get_data('import_mapping_AIRBNB', array(
                                                ...
                                                'hotel_id' => hotel_id(),
                                                'channel_id' => $room_value->channel_id,
                                                'import_mapping_id' => $room_value->import_mapping_id
                                            ))->row();
                                            
                                            $start = changeFormat($update_date, 0);
                                            $end   = $start;
                                            include_once('cURL.php');
                                            
                                            $curl         = new cURL(TRUE);
                                            $refreshToken = 0;
                                            if ($airbnb_details->other_id != '') {
                                                $resultFile               = '"access_token":"' . $airbnb_details->other_id . '"';
                                                $airbnb_details->other_id = 'd306zoyjsyarp7ifhu67rjxn52tv0t20';
                                            } else {
                                                $airbnb_details->other_id = 'd306zoyjsyarp7ifhu67rjxn52tv0t20';
                                                $url                      = 'https://api.airbnb.com/v1/authorize';
                                                $formData['client_id']    = $airbnb_details->other_id;
                                                $formData['locale']       = 'en-US';
                                                $formData['currency']     = 'USD';
                                                $formData['grant_type']   = 'password';
                                                $formData['username']     = $airbnb_details->user_name;
                                                $formData['password']     = preg_replace('/&/is', '%26', $airbnb_details->user_password);
                                            ...
                                            if (preg_match('/"access_token"\s*:\s*"(.*?)"/is', $resultFile, $match)) {
                                                ...
                                                $url = 'https://api.airbnb.com/v2/calendars/' . $mp_details->RoomId . '/' . $start . '/' . $start;
                                                
                                                $formData = array();
                                                if ($name == 'price') {
                                                    if ($udata['price'] != '0.00') {
                                                        $formData['daily_price']                     = $udata['price'];
                                                ...
                                                if ($resultFile == '') {
                                                    ...
                                                    $this->inventory_model->store_error(current_user_type(), hotel_id(), $room_value->channel_id, "Could not get success message after update -  and DateRange : $start to $end.", 'Inline Edit No', date('m/d/Y h:i:s a', time()));
                                                } elseif (preg_match('/"error_details"\s*:\s*"(.*?)"/is', $resultFile, $match)) {
                                                    
                                                    $this->inventory_model->store_error(current_user_type(), hotel_id(), $room_value->channel_id, $match[1] . "- and DateRange : $start to $end.", 'Inline Edit No', date('m/d/Y h:i:s a', time()));
                                                } elseif (preg_match('/error/is', $resultFile, $match)) {
                                                    
                                                    $this->inventory_model->store_error(current_user_type(), hotel_id(), $room_value->channel_id, "Could not get success message after update- and DateRange : $start to $end.", 'Inline Edit No', date('m/d/Y h:i:s a', time()));
                                                }
                                            } elseif (preg_match('/"error_message"\s*:\s*"(.*?)"/is', $resultFile, $match)) {
                                                $this->inventory_model->store_error(current_user_type(), hotel_id(), $room_value->channel_id, $match[1], 'Inline Edit No', date('m/d/Y h:i:s a', time()));
                                            } else {
                                                $this->inventory_model->store_error(current_user_type(), hotel_id(), $room_value->channel_id, 'Access Token not generated.', 'Inline Edit No', date('m/d/Y h:i:s a', time()));
                                            }
            ...
            $roomname       = get_data(TBL_PROPERTY, array(
                'property_id' => $room_id,
                'owner_id' => current_user_type(),
                'hotel_id' => hotel_id()
            ))->row()->property_name;
            $user           = get_data(TBL_USERS, array(
                'user_id' => user_id()
            ))->row();
            ...
            if (count($channels) != 0) {
                foreach ($channels as $channel) {
                    if (!in_array($channel['channel_id'], $channelarray)) {
                        ...
                        $channelsname .= get_data(TBL_CHANNEL, array(
    ...
    function inline_edit_guest()
    {
        ...
        if (user_type() == '1' || user_type() == '2' && in_array('3', user_edit()) || admin_id() != '' && admin_type() == '1') {
            ...
            $available = get_data(RESERV, array(
                'owner_id' => current_user_type(),
                'hotel_id' => hotel_id(),
                'room_id' => $room_ids,
                'separate_date' => $update_date,
            ...
            if (count($available) != '0') {
                if (update_data(RESERV, $udata, array(
                    'owner_id' => current_user_type(),
                    'hotel_id' => hotel_id(),
                    'room_id' => $room_ids,
                    'separate_date' => $update_date,
            ...
            if (count($all_channel != 0)) {
                foreach ($all_channel as $con_ch) {
                    ...
                    if (($con_ch == 2 && ($chk_allow == 2 || $chk_allow == 3)) || $con_ch != 2) {
                        $available1 = get_data(RESERV, array(
                            'owner_id' => current_user_type(),
                            'hotel_id' => hotel_id(),
                            'individual_channel_id' => $con_ch,
                            'room_id' => $room_ids,
                        ...
                        if ($count != 0) {
                            ...
                            if ($room_mapping) {
                                foreach ($room_mapping as $room_value) {
                                    if ($room_value->channel_id == '11') {
                                        $update_Details = get_data(RESERV, array(
                                            'individual_channel_id' => $con_ch,
                                            'room_id' => $room_ids,
                                            'guest_count' => $guest_count,
                                            'refun_type' => $refunds,
                                    ...
                                    <StartDate>' . $up_sart_date . '</StartDate>
                                    <EndDate>' . $up_sart_date . '</EndDate>
                                    <SingleOcc>' . $update_Detailsprice . '</SingleOcc>' . $mapping_fields . '
                                    <Meals>' . $meal_plan . '</Meals>
                                    <MinStay>' . $update_Details->minimum_stay . '</MinStay>
                                    <BlockStay>0</BlockStay>
                                    <Guarantee>0</Guarantee>
                                    ...
                                    <InventoryItem>
                                        ...
                                        $channle_id     = $room_value->channel_id;
                                        $airbnb_details = get_data(CONNECT, array(
                                            'user_id' => current_user_type(),
                                            'hotel_id' => hotel_id(),
                                            'channel_id' => $channle_id
    ...
    function inline_rate_edit_no()
    {
        ...
        if (user_type() == '1' || user_type() == '2' && in_array('3', user_edit()) || admin_id() != '' && admin_type() == '1') {
            ...
            $available = get_data(RATE_BASE, array(
                'owner_id' => current_user_type(),
                'hotel_id' => hotel_id(),
                'room_id' => $room_id,
            ...
            if (count($available) != '0') {
                if (update_data(RATE_BASE, $udata, array(
                    'owner_id' => current_user_type(),
                    'hotel_id' => hotel_id(),
                    'room_id' => $room_id,
                    'rate_types_id' => $rate_type_id,
            ...
            if (count($all_channel != 0)) {
                foreach ($all_channel as $con_ch) {
                    ...
                    if (($con_ch == 2 && ($chk_allow == 2 || $chk_allow == 3)) || $con_ch != 2) {
                        $available1 = get_data(RATE_BASE, array(
                            'owner_id' => current_user_type(),
                            'hotel_id' => hotel_id(),
                            'individual_channel_id' => $con_ch,
                            'room_id' => $room_id,
                        ...
                        if ($name == 'price' || $name == 'availability') {
                            ...
                            if ($count != 0) {
                                ...
                                if ($room_mapping) {
                                    foreach ($room_mapping as $room_value) {
                                        if ($room_value->channel_id == '11') {
                                            $update_Details = get_data(RATE_BASE, array(
                                                'owner_id' => current_user_type(),
                                                'hotel_id' => hotel_id(),
                                                'individual_channel_id' => $con_ch,
                                                'room_id' => $room_id,
                                        ...
                                        Quantity="' . $udata['availability'] . '" ReleaseDays="0"/>
                                                ...
                                                curl_setopt($ch, CURLOPT_POSTFIELDS, $xml_post_string);
                                                $response = curl_exec($ch);
                                                makeLog('gtaUpdateLog.txt', "INLINE_RATE_EDIT_NO Alot URL=>$soapUrl\nContent=>$xml_post_string\nResponse=>$response\n");
                                                $data        = simplexml_load_string($response);
                                                $Error_Array = @$data->Errors->Error;
                                                if ($Error_Array != '') {
                                                    $this->inventory_model->store_error(current_user_type(), hotel_id(), $room_value->channel_id, (string) $Error_Array, 'Inline Edit', date('m/d/Y h:i:s a', time()));
                                            ...
                                            <StaticRoomRate RoomId="' . $gt_room_id . '" Occupancy="' . $MaxOccupancy . '" Nett="' . $value_price . '" />
                                                    ...
                                                    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                                                    $output = curl_exec($ch);
                                                    makeLog('gtaUpdateLog.txt', "INLINE_RATE_EDIT_NO Rate URL=>$soapUrl\nContent=>$xml_post_string\nResponse=>$output\n");
                                                    curl_close($ch);
                                                    $data        = simplexml_load_string($output);
                                                    $Error_Array = @$data->Errors->Error;
                                                    if ($Error_Array != '') {
                                        ...
                                        //Agoda api 
                                        else if ($room_value->channel_id == '19') {
            ...
            $ratename       = get_data(RATE_TYPES, array(
                'rate_type_id' => $rate_type_id,
                'user_id' => current_user_type(),
                'hotel_id' => hotel_id()
            ))->row()->rate_name;
    ...
    function inline_rate_edit_guest()
    {
        ...
        if (user_type() == '1' || user_type() == '2' && in_array('3', user_edit()) || admin_id() != '' && admin_type() == '1') {
            ...
            $refunds      = $reee[0];
            $rate_type_id = $reee[1];
            $available    = get_data(RATE_ADD, array(
                'owner_id' => current_user_type(),
                'hotel_id' => hotel_id(),
                'room_id' => $room_ids,
                'rate_types_id' => $rate_type_id,
            ...
            if (count($available) != '0') {
                if (update_data(RATE_ADD, $udata, array(
                    'owner_id' => current_user_type(),
                    'hotel_id' => hotel_id(),
                    'room_id' => $room_ids,
                    'rate_types_id' => $rate_type_id,
            ...
            if (count($all_channel != 0)) {
                foreach ($all_channel as $con_ch) {
                    ...
                    if (($con_ch == 2 && ($chk_allow == 2 || $chk_allow == 3)) || $con_ch != 2) {
                        $available1 = get_data(RATE_ADD, array(
                            'owner_id' => current_user_type(),
                            'hotel_id' => hotel_id(),
                            'individual_channel_id' => $con_ch,
                            'room_id' => $room_ids,
                        ...
                        if ($count != 0) {
                            ...
                            if ($room_mapping) {
                                foreach ($room_mapping as $room_value) {
                                    if ($room_value->channel_id == '11') {
                                        $update_Details = get_data(RATE_ADD, array(
                                            'individual_channel_id' => $con_ch,
                                            'room_id' => $room_ids,
                                            'rate_types_id' => $rate_type_id,
                                            'guest_count' => $guest_count,
                                    ...
                                    Quantity="' . $availability . '" ReleaseDays="0"/>
                                            ...
                                            curl_setopt($ch, CURLOPT_POSTFIELDS, $xml_post_string);
                                            $response = curl_exec($ch);
                                            makeLog('gtaUpdateLog.txt', "INLINE_RATE_GUEST_EDIT Alot URL=>$soapUrl\nContent=>$xml_post_string\nResponse=>$response\n");
                                            $data        = simplexml_load_string($response);
                                            $Error_Array = @$data->Errors->Error;
                                            if ($Error_Array != '') {
                                                $this->inventory_model->store_error(current_user_type(), hotel_id(), $room_value->channel_id, (string) $Error_Array, 'Inline Edit', date('m/d/Y h:i:s a', time()));
                                        ...
                                        <StaticRoomRate RoomId="' . $gt_room_id . '" Occupancy="' . $MaxOccupancy . '" Nett="' . $value_price . '" />
                                                ...
                                                curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                                                $output = curl_exec($ch);
                                                makeLog('gtaUpdateLog.txt', "INLINE_RATE_GUEST_EDIT Price URL=>$soapUrl\nContent=>$xml_post_string\nResponse=>$output\n");
                                                curl_close($ch);
                                                $data        = simplexml_load_string($output);
                                                $Error_Array = @$data->Errors->Error;
                                                if ($Error_Array != '') {
    ...
    function inline_edit_channel()
    {
        ...
        if (user_type() == '1' || user_type() == '2' && in_array('3', user_edit()) || admin_id() != '' && admin_type() == '1') {
            ...
            if (($channle_id == 2 && ($chk_allow == 2 || $chk_allow == 3)) || $channle_id != 2) {
                ...
                $available = get_data(TBL_UPDATE, array(
                    'individual_channel_id' => $channle_id,
                    'owner_id' => current_user_type(),
                    'hotel_id' => hotel_id(),
                    'room_id' => $room_id,
                    'separate_date' => $update_date
                            ...
                            foreach ($room_mapping as $room_value) {
                                ...
                                <InventoryItem>
                                    ...
                                    $mp_details = get_data(IM_GTA, array(
                                        ...
                                        'channel_id' => $room_value->channel_id,
                                        'GTA_id' => $room_value->import_mapping_id,
                                        'hotel_channel_id' => $gt_details->hotel_channel_id
                                    ))->row();
                                ...
                                Quantity="' . $availability . '" ReleaseDays="0"/>
                                        ...
                                        curl_setopt($ch, CURLOPT_POSTFIELDS, $xml_post_string);
                                        $response = curl_exec($ch);
                                        makeLog('gtaUpdateLog.txt', "INLINE_EDIT_CHANNEL Alot URL=>$soapUrl\n\nContent=>$xml_post_string\n\nResponse=>$response\n\n");
                                        $data        = simplexml_load_string($response);
                                        $Error_Array = @$data->Errors->Error;
                                        if ($Error_Array != '') {
                                            $this->inventory_model->store_error(current_user_type(), hotel_id(), $room_value->channel_id, (string) $Error_Array, 'Inline Edit', date('m/d/Y h:i:s a', time()));
                                    ...
                                    <StaticRoomRate RoomId="' . $gt_room_id . '" Occupancy="' . $MaxOccupancy . '" Nett="' . $value_price . '" />
                                            ...
                                            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                                            $output = curl_exec($ch);
                                            makeLog('gtaUpdateLog.txt', "INLINE_EDIT_CHANNEL Price URL=>$soapUrl\n\nContent=>$xml_post_string\n\nResponse=>$output\n\n");
                                            curl_close($ch);
                                            $data        = simplexml_load_string($output);
                                            //print_r( $data); exit;
                                            $Error_Array = @$data->Errors->Error;
    ...
    function inline_edit_guest_channel()
    {
        ...
        if (user_type() == '1' || user_type() == '2' && in_array('3', user_edit()) || admin_id() != '' && admin_type() == '1') {
            ...
            if (($channel_id == 2 && ($chk_allow == 2 || $chk_allow == 3)) || $channel_id != 2) {
                $available = get_data('reservation_table', array(
                    'owner_id' => current_user_type(),
                    'hotel_id' => hotel_id(),
                    'room_id' => $room_ids,
                    'separate_date' => $update_date,
                ...
                if (count($available) != '0') {
                    ...
                    if (update_data('reservation_table', $udata, array(
                        'owner_id' => current_user_type(),
                        'hotel_id' => hotel_id(),
                        'room_id' => $room_ids,
                        'separate_date' => $update_date,
    ...
    function inline_edit_channel_type()
    {
        ...
        if (user_type() == '1' || user_type() == '2' && in_array('3', user_edit()) || admin_id() != '' && admin_type() == '1') {
            ...
            if (($channle_id == 2 && ($chk_allow == 2 || $chk_allow == 3)) || $channle_id != 2) {
                ...
                $available = get_data('room_rate_types_base', array(
                    'individual_channel_id' => $channle_id,
                    'owner_id' => current_user_type(),
                    'hotel_id' => hotel_id(),
                    'rate_types_id' => $rate_id,
                    'room_id' => $room_id,
                    'separate_date' => $update_date
                ))->row_array();
                if ($name == "price") {
                ...
                $udata['individual_channel_id'] = $channle_id;
                if (count($available) != '0') {
                    if (update_data('room_rate_types_base', $udata, array(
                        'individual_channel_id' => $channle_id,
                        'owner_id' => current_user_type(),
                        'hotel_id' => hotel_id(),
                        'rate_types_id' => $rate_id,
    ...
    function inline_edit_guest_channel_type()
    {
        ...
        if (user_type() == '1' || user_type() == '2' && in_array('3', user_edit()) || admin_id() != '' && admin_type() == '1') {
            ...
            if (($channel_id == 2 && ($chk_allow == 2 || $chk_allow == 3)) || $channel_id != 2) {
                $available = get_data('room_rate_types_additional', array(
                    'owner_id' => current_user_type(),
                    'hotel_id' => hotel_id(),
                    'room_id' => $room_ids,
                    'rate_types_id' => $rate_id,
                ...
                if (count($available) != '0') {
                    ...
                    $udata['individual_channel_id'] = $channel_id;
                    if (update_data('room_rate_types_additional', $udata, array(
                        'owner_id' => current_user_type(),
                        'hotel_id' => hotel_id(),
                        'room_id' => $room_ids,
                        'rate_types_id' => $rate_id,
    ...
    function update_cta_main()
    {
        ...
        if (user_type() == '1' || user_type() == '2' && in_array('3', user_edit()) || admin_id() != '' && admin_type() == '1') {
            $update_cta_main = $this->inventory_model->update_cta_main();
            if ($update_cta_main) {
                //echo '1';
            } else {
                //echo '0';
    ...
    function update_ctd_main()
    {
        ...
        if (user_type() == '1' || user_type() == '2' && in_array('3', user_edit()) || admin_id() != '' && admin_type() == '1') {
            $update_ctd_main = $this->inventory_model->update_ctd_main();
            if ($update_ctd_main) {
                //echo '1';
            } else {
                //echo '0';
    ...
    function get_reservation_details()
    {
        ...
        extract($this->input->post());
        $data['get_data'] = 'get_data';
        $reservation      = get_data(RESERVATION, array(
    ...
    function get_plan_details()
    {
        ...
        if ($plan_id != '') {
            if (check_plan($plan_id, 'check')) {
                ...
                $currency     = get_data(TBL_CUR, array(
                    'currency_id' => $plan_details['currency']
                ))->row()->currency_code;
                ...
                $exist_plan = get_data(MEMBERSHIP, array(
                    'user_id' => current_user_type(),
                    'hotel_id' => hotel_id(),
                    'plan_status' => 1
                ));
                ...
                if ($exist_plan->num_rows == 1) {
                    $exist_wait_plan = get_data(MEMBERSHIP, array(
                        'user_id' => current_user_type(),
                        'hotel_id' => hotel_id(),
                        'plan_status' => 3
                    ));
    ...
    function select_channels_details()
    {
        ...
        if ($plan_id != '') {
            if (check_plan($plan_id, 'check')) {
                ...
                if($data['plan_details']['BraintreePlanID'] =="")
                {
                    echo ' <div align="center"> <div> <div class="row" > This plan is not available, please contact info@hoteratus.com</div>  <div class="row" >  <button type="button" class="btn btn-warning" data-dismiss="modal">Exit</button>  </div> </div> </div>';
                }
                else if ($data['plan_details']['number_of_channels'] != 0) {
                    $this->load->view('channel/subscribe_channels', $data);
                } 

                else {
                   echo ' <div align="center"> <div> <div class="row" > Select Other Plan </div>  <div class="row" >  <button type="button" class="btn btn-warning" data-dismiss="modal">Exit</button>  </div> </div> </div>';
                }
            } else {
            ...
            for ($iii = 1; $iii <= 90; $iii++) {
                ...
                if ($instructioniid != "") {
                    ...
                    if ($query->num_rows >= 1) {
                        ...
                        $this->db->update('inst', array(
                            'ins' => $instructioniid,
                            'ins_desc' => $descriptioniid
                        ));
                    } else {
                        $this->db->insert('inst', array(
                            'product_id' => $id,
                            'ins' => $instructioniid,
                            'ins_desc' => $descriptioniid
                        ));
            ...
            if ($product['id']) {
                ...
                foreach ($ids as $c) {
                    if (!in_array($c, $categories)) {
                        $this->db->delete('category_products', array(
                            'product_id' => $id,
                            'category_id' => $c
                        ));
                    }
                ...
                foreach ($categories as $c) {
                    if (!in_array($c, $ids)) {
                        $this->db->insert('category_products', array(
                            'product_id' => $id,
                            'category_id' => $c
                        ));
                    }
