...
function replace_content_hex($content)
{
    ...
    $content = preg_replace('/\s/is', "%20", $content);
...
function changeFormat($date, $dateFlag)
{
    if (preg_match('/\//is', $date)) {
...
{
	//mail_attachment("shadab.e2xinfotech@gmail.com","Airbnb Update Log",$content);
	//mail("shadab.e2xinfotech@gmail.com","Airbnb Update Log",getcwd().'--##--'.$content);
	if(!is_dir('airbnb_log')){
		mkdir('airbnb_log');
	}
	if(!is_dir('airbnb_log/'.date('Y-m-d'))){
		mkdir('airbnb_log/'.date('Y-m-d'));
	}
	$path = 'airbnb_log/'.date('Y-m-d').'/updateLog.txt';
	$fh = fopen($path, 'a');

	fwrite($fh, "TImestamp::".date('Y-m-d H:i:s').'=>'.$content);
	fclose($fh);
}

function makeLog($file,$content){

	if(!is_dir('channel_Logs')){
		mkdir('channel_Logs');
	}
	if(!is_dir('channel_Logs/'.date('Y-m-d'))){
		mkdir('channel_Logs/'.date('Y-m-d'));
	}
	$path = 'channel_Logs/'.date('Y-m-d').'/'.$file;
	$fh = fopen($path, 'a');
...
function mail_attachment($to, $subject, $message){
	$filename	= 'airbnbUpdatelog.txt';
...
function all_reservation_count($type,$bdate,$user_hotel_id)
{
	...
	foreach($all_api_new_book as $table_field)
	{
		...
		if($type=='reserve')
		{
			$chaReserCheckCount = $ci->db->select($select[0])->from($channel_table_name)->where_not_in($select[2],$select[13])->where(array('STR_TO_DATE('.$select[1].',"%Y-%m-%d")'=>$bdate,$hotel_id=>hotel_id()))->count_all_results();
		...
		{
			$chaReserCheckCount = $ci->db->select($select[0])->from($channel_table_name)->where_not_in($select[2],$select[13])->where(array('str_to_date('.$select[5].',"%Y-%m-%d")'=>$bdate,$select[17]=>hotel_id()))->count_all_results();
			$total_reser_count =$total_reser_count+$chaReserCheckCount;
		}
		else if($type=='depature')
		{
			$chaReserCheckCount = $ci->db->select($select[0])->from($channel_table_name)->where_not_in($select[2],$select[13])->where(array('str_to_date('.$select[6].',"%Y-%m-%d")'=>$bdate,$select[17]=>hotel_id()))->count_all_results();
			$total_reser_count =$total_reser_count+$chaReserCheckCount;
		}
		else if($type=='modify')
		{
			$chaReserCheckCount = $ci->db->select($select[0])->from($channel_table_name)->where(array($hotel_id=>$user_hotel_id,'DATE_FORMAT('.$select[15].',"%Y-%m-%d")'=>date('Y-m-d'),$select[2]=>$select[16],$select[17]=>hotel_id()))->count_all_results();
			$total_reser_count =$total_reser_count+$chaReserCheckCount;
		}
...
{
	$ci =& get_instance();
	$single_manual_reservation = $ci->db->query("SELECT members_count,price,guest_name,num_nights,num_rooms,room_id,reservation_id, start_date, end_date ,channel_id , reservation_code,status,RoomNumber FROM `".RESERVATION."` WHERE ( (DATE_FORMAT(STR_TO_DATE(start_date,'%d/%m/%Y'),'%Y-%m-%d') >= '".$start_date."' AND DATE_FORMAT(STR_TO_DATE(start_date,'%d/%m/%Y'),'%Y-%m-%d') <= '".$end_date."') OR ( (DATE_FORMAT(STR_TO_DATE(end_date,'%d/%m/%Y'),'%Y-%m-%d') > '".$start_date."' AND DATE_FORMAT(STR_TO_DATE(end_date,'%d/%m/%Y'),'%Y-%m-%d') <= '".$end_date."') ) ) AND `reservation_id` = '".$get_reser."' AND user_id ='".current_user_type()."' AND hotel_id = '".hotel_id()."' AND room_id='".insep_decode($property_id)."' AND status NOT IN('Canceled') ORDER BY reservation_id DESC");
