...
class inventory_model extends CI_Model
{
    ...
    function save($product, $options = false, $categories = false, $resdata = array())
    {
        ...
        if (is_array($resdata) and count($resdata) > 0) {
            $mainCall       = $resdata['maincal'];
            $all_channel_id = $resdata['channel_id'];
            //print "mainCall=>$mainCall";
            //print "all_channel_id=>";print_r($all_channel_id);
            //print "period=>";print_r($period);
        } else {
            $mainCall       = $this->input->post('maincal');
            ...
            foreach ($period as $date) {
                ...
                if (count($available1) != 0) {
                    ...
                    if (isset($product['cta']) != '') {
                        $product_avail['cta'] = ($product['cta']=='2'?0:$product['cta']);
                    }
                    
                    if (isset($product['ctd']) != '') {
                        $product_avail['ctd'] = ($product['ctd']=='2'?0:$product['ctd']);
                    }
        ...
        if ($all_channel_id) {
            foreach ($all_channel_id as $channel_id) {
                ...
                if($channel_id==19)
                {
                    ...
                    if ($room_mapping) {
                        if (array_key_exists(0, $room_mapping)) {
                            $rate_conversion = $room_mapping[0]->rate_conversion;
                        ...
                        $this->agoda_model->bulk_update($product, $room_mapping[0]->import_mapping_id, $room_mapping[0]->mapping_id, $price);
                ...
                if ($channel_id == 8) {
                    ...
                    if ($room_mapping) {
                        ...
                        $this->load->model("gta_model");
                        $this->gta_model->bulk_update($product, $room_mapping[0]->import_mapping_id, $room_mapping[0]->mapping_id, $price);
                    }
                }
                if ($channel_id == 9) {
                    //echo $product['room_id'];
                    $room_mapping = get_data(MAP, array(
                        'owner_id' => current_user_type(),
                        'hotel_id' => hotel_id(),
                        'channel_id' => $channel_id,
                    ...
                    if ($room_mapping) {
                        ...
                        $this->airbnb_model->bulk_update($product, $room_mapping[0]->import_mapping_id, $room_mapping[0]->mapping_id, $price);
                    }
                }
                                        ...
                                        foreach ($room_mapping as $room_value) {
                                            ...
                                            if ($room_value->channel_id == '11') {
                                                if ($value_price != '0' || $ch_update_avail != '') {
                                                    ...
                                                    <SingleOcc>' . $value_price . '</SingleOcc>' . $mapping_fields . '
                                            ...
                                            } else if ($room_value->channel_id == '9') {
                                                $value['start_date'] = $this->input->post('start_date');
                                                $value['end_date']   = $this->input->post('end_date');
                                                $value['price']      = @$value_price;
                                                $value['days']       = $sub_rooms['days'];
                                                $this->load->model("airbnb_model");
                                                $this->airbnb_model->bulk_update($value, $stop_sale, $room_value->import_mapping_id, $room_value->mapping_id);
                ...
                foreach ($channel_id as $con_channel) {
                    foreach ($clean as $id => $rooms) {
                        ...
                        foreach ($rooms as $rate_types_id => $room) {
                            $rate_details = get_data(RATE_TYPES, array(
                                'user_id' => current_user_type(),
                                'hotel_id' => hotel_id(),
                                'rate_type_id' => $rate_types_id,
                                'room_id' => $id
                            ))->row();
                            if (count($rate_details) != 0) {
                                if ($rate_details->meal_plan == '1' || $rate_details->meal_plan == '0') {
                                    $meal_plan = 0;
                                } elseif ($rate_details->meal_plan == '2') {
                                    $meal_plan = 1;
                                } elseif ($rate_details->meal_plan == '3') {
                                    $meal_plan = 3;
                                } elseif ($rate_details->meal_plan == '4') {
                                    $meal_plan = 0;
                                } elseif ($rate_details->meal_plan == '5' || $rate_details->meal_plan == '6') {
                                    $meal_plan = 2;
                                }
                            } else {
                                $meal_plan = 0;
                                        ...
                                        if ($room_mapping) {
                                            foreach ($room_mapping as $room_value) {
                                                ...
                                                if ($room_value->channel_id == '11') {
                                                    ...
                                                    $this->load->model("airbnb_model");
                                                    $this->airbnb_model->update_stopsell($un_date, $room_value->property_id, $room_value->import_mapping_id);
                                                }
                            ...
                            foreach ($period as $date) {
                                ...
                                if ($up_channel_id != '') {
                                    foreach ($up_channel_id as $con_channel) {
                                        ...
                                        if (($con_channel == 2 && ($chk_allow == 2 || $chk_allow == 3)) || $con_channel != 2) {
                                            if (user_type() == '1' || admin_id() != '' && admin_type() == '1') {
                                                $ch_available = get_data(RATE_ADD, array(
                                                    'owner_id' => user_id(),
                                                    'hotel_id' => hotel_id(),
                                                    'room_id' => $id,
                                                    'rate_types_id' => $rate_types_id,
                                                    'guest_count' => $guest_count,
                                                    'refun_type' => $refun,
                                                    'individual_channel_id' => $con_channel,
                                                    'separate_date' => $date
                                                ))->row_array();
                                            } else if (user_type() == '2') {
                                                $ch_available = get_data(RATE_ADD, array(
    ...
    function update_stopsell_main()
    {
        ...
        if ($this->input->post('alter_checkbox') != '') {
            ...
            foreach ($exp as $val) {
                $array_alter1 = str_replace(array(
                    'room',
                    '][',
                    '[',
                    ']'
                ...
                $s_available = get_data(TBL_UPDATE, array(
                    'owner_id' => current_user_type(),
                    'hotel_id' => hotel_id(),
                    'room_id' => $un_roomid,
                    'individual_channel_id' => 0,
                    'separate_date' => $un_date
                ), $un_column)->row_array();
                ...
                if (count($s_available) != 0) {
                    ...
                    $update = 'stop_sell';
                    $column = 'stop_sell';
                    if (update_data(TBL_UPDATE, $undata, array(
                        'owner_id' => current_user_type(),
                        'hotel_id' => hotel_id(),
                        'room_id' => $un_roomid,
                        'individual_channel_id' => 0,
                ...
                if (count($all_channel) != "") {
                    foreach ($all_channel as $con_ch) {
                        ...
                        if (($con_ch == 2 && ($chk_allow == 2 || $chk_allow == 3)) || $con_ch != 2) {
                            $ch_available = get_data(TBL_UPDATE, array(
                                'owner_id' => current_user_type(),
                                'hotel_id' => hotel_id(),
                            ...
                            if (count($ch_available) == 0) {
                                ...
                                $c_available = get_data(TBL_UPDATE, array(
                                    'owner_id' => current_user_type(),
                                    'hotel_id' => hotel_id(),
                                    'room_id' => $un_roomid,
                                    'individual_channel_id' => 0,
                        ...
                        foreach ($room_mapping as $room_value) {
                            if ($room_value->channel_id == '11') {
                                $availability_details = get_data(TBL_UPDATE, array(
                                    'owner_id' => current_user_type(),
                                    'hotel_id' => hotel_id(),
                                    'individual_channel_id' => $room_value->channel_id,
                                    'room_id' => $room_value->property_id,
                                    'separate_date' => $un_date
                                ))->row();
                                ...
                                if (isset($data->Success)) {
                                    /*$gtaresponse="success";
                                    $this->session->set_flashdata('bulk_error',$this->session->flashdata('bulk_error').'##'.(string)$gtaresponse);
                                    echo "OK";  */
                                } else {
                                    
                                    /*   $this->session->set_flashdata('bulk_error',$this->session->flashdata('bulk_error').'##'."Bulk update success");
                                    echo "Failed"; */
                                }
                                curl_close($ch);
                    ...
                    if ($room_mapping) {
                        foreach ($room_mapping as $room_value) {
                            if ($room_value->channel_id == '2') {
                                ...
                                if ($chk_allow == 2 || $chk_allow == 3) {
                                    $this->load->model("booking_model");
                                    $this->booking_model->update_cta($un_date, $room_value->property_id, $room_value->import_mapping_id);
                                }
                            }
                        }
                    }
                    ...
                    if ($room_mapping) {
                        foreach ($room_mapping as $room_value) {
                            if ($room_value->channel_id == '2') {
                                ...
                                if ($chk_allow == 2 || $chk_allow == 3) {
                                    $this->load->model("booking_model");
                                    $this->booking_model->update_ctd($un_date, $room_value->property_id, $room_value->import_mapping_id);
                                }
                            }
                        }
                    }
        ...
        while ($dias > 0) {
            ...
            if ($optionsRadios == 'select') {
                ...
                foreach ($channel_id as $Channelid) {
                    if ($Channelid == 36) {
                        $this->load->model("despegar_model");
                        $DespegarErrors .= $this->despegar_model->SincroCalender($inicialdate, $FinalDate,$userid, $hotelid);
                        
                        
                    } elseif ($Channelid == 1) {
                        $this->load->model("expedia_model");
                        $ExpediaErrors .= $this->expedia_model->SincroCalender($inicialdate, $FinalDate,$userid, $hotelid,'All');
                        
                    } elseif ($Channelid == 2) {
                        
                        $this->load->model("booking_model");
                        $BookingErrors .= $this->booking_model->SincroCalender($inicialdate, $FinalDate,$userid, $hotelid);


                    } elseif ($Channelid == 9) {
                        
                        $this->load->model("airbnb_model");
                        $AirbnbErrors .= $this->airbnb_model->SincroCalender($inicialdate, $FinalDate,$userid, $hotelid);
                    }elseif ($Channelid == 19) {
                        
                        $this->load->model("agoda_model");
                        $AgodaErrors .= $this->agoda_model->SincroCalender($inicialdate, $FinalDate,$userid, $hotelid);
                    }
                }
        ...
        if (($con_ch == 2 && ($chk_allow == 2 || $chk_allow == 3)) || $con_ch != 2) {
            if ($this->input->post('alter_checkbox') != '') {
                ...
                foreach ($exp as $val) {
                    ...
                    if (update_data(TBL_UPDATE, $undata, array(
                        'individual_channel_id' => $this->input->post('channe_id_update'),
                        'owner_id' => current_user_type(),
                        'hotel_id' => hotel_id(),
                        'room_id' => $un_roomid,
                        'separate_date' => $un_date
                    ))) {
                        $updated_data = get_data(TBL_UPDATE, array(
                            'individual_channel_id' => $this->input->post('channe_id_update'),
                            'owner_id' => current_user_type(),
                            'hotel_id' => hotel_id(),
                            'room_id' => $un_roomid,
            ...
            if ($this->input->post('alter_checkbox_refund') != '') {
                ...
                foreach ($exp as $val) {
                    ...
                    if (update_data(RESERV, $undata, array(
                        'individual_channel_id' => $this->input->post('channe_id_update'),
                        'owner_id' => current_user_type(),
                        'hotel_id' => hotel_id(),
                        'room_id' => $un_roomid,
                        'separate_date' => $un_date,
                        'guest_count' => $un_guest,
                        'refun_type' => $un_refund
                    ))) {
                        $updated_data = get_data(RESERV, array(
                            'individual_channel_id' => $this->input->post('channe_id_update'),
                            'owner_id' => current_user_type(),
                            'hotel_id' => hotel_id(),
                            'room_id' => $un_roomid,
            ...
            if ($this->input->post('alter_checkbox_rate') != '') {
                ...
                foreach ($exp as $val) {
                    ...
                    if (update_data(RATE_BASE, $undata, array(
                        'individual_channel_id' => $this->input->post('channe_id_update'),
                        'owner_id' => current_user_type(),
                        'hotel_id' => hotel_id(),
                        'room_id' => $un_roomid,
                        ...
                        $updated_data = get_data(RATE_BASE, array(
                            'individual_channel_id' => $this->input->post('channe_id_update'),
                            'owner_id' => current_user_type(),
                            'hotel_id' => hotel_id(),
                            'room_id' => $un_roomid,
            ...
            if ($this->input->post('alter_checkbox_rate_refund') != '') {
                ...
                foreach ($exp as $val) {
                    ...
                    if (update_data(RATE_ADD, $undata, array(
                        'individual_channel_id' => $this->input->post('channe_id_update'),
                        'owner_id' => current_user_type(),
                        'hotel_id' => hotel_id(),
                        'room_id' => $un_roomid,
                        'rate_types_id' => $un_rate_type_id,
                        'separate_date' => $un_date,
                        'guest_count' => $un_guest,
                        'refun_type' => $un_refund
                    ))) {
                        $update_data = get_data(RATE_ADD, array(
                            'individual_channel_id' => $this->input->post('channe_id_update'),
                            'owner_id' => current_user_type(),
                            'hotel_id' => hotel_id(),
                            'room_id' => $un_roomid,
            ...
            */
            if (isset($new_room)) {
                foreach ($new_room as $room_id => $rooms) {
                    $available_dates = get_data(TBL_UPDATE, array(
                        'individual_channel_id' => $channe_id_update,
                        'owner_id' => current_user_type(),
                        'hotel_id' => hotel_id(),
                        'room_id' => $room_id
                    ))->result_array();
            ...
            if (isset($new_room_rate)) {
                ...
                foreach ($new_room_rate as $room_id => $room) {
                    foreach ($room as $rate_type_id => $rooms) {
                        $available_dates = get_data(RATE_BASE, array(
                            'individual_channel_id' => $channe_id_update,
                            'owner_id' => current_user_type(),
                            'hotel_id' => hotel_id(),
                            'room_id' => $room_id,
    ...
    function payment_success_mail($payment_id)
    {
        ...
        if (insep_decode($payment_id) == 1) {
            $plan_names = get_data(TBL_PLAN, array(
                'plan_id' => insep_decode($payment_id)
            ))->row();
            $plan_name  = $plan_names->plan_price . ' ' . $plan_names->plan_name;
        } else {
            $plan_name = get_data(TBL_PLAN, array(
                'plan_id' => insep_decode($payment_id)
            ))->row()->plan_name;
        }
        
        $row = get_data(TBL_USERS, array(
            'user_id' => current_user_type()
        ))->row();
        ...
        $admin_mail    = get_data('site_config', array(
            'id' => '1'
        ))->row();
    ...
    {
        if (user_type() == '1' || admin_id() != '' && admin_type() == '1') {
            $count = $this->db->select('photo_id')->from(TBL_PHOTO)->where(array(
                'user_id' => user_id(),
                'hotel_id' => hotel_id(),
    ...
    {
        // if(user_type()=='1'){
        $ho_id = get_data(TBL_PHOTO, array(
            'user_id' => current_user_type(),
            'hotel_id' => hotel_id(),
            'room_id' => insep_decode($this->input->post('hotel_id'))
        ))->row_array();
        /*}
        else if(user_type()=='2'){
        ...
        if (count($ho_id) != 0) {
            ...
            // if(user_type()=='1'){
            $room_photo           = update_data(TBL_PHOTO, $udata, array(
                'user_id' => current_user_type(),
                'hotel_id' => hotel_id(),
                'room_id' => insep_decode($this->input->post('hotel_id'))
            ));
    ...
    {
        $count = $this->db->select('rate_type_id')->from(RATE_TYPES)->where(array(
            'room_id' => insep_decode($property_id)
        ))->count_all_results();
        return $count;
    }
    ...
    {
        $roomMappingCheck = get_data(MAP, array(
            'property_id' => insep_decode($property_id)
        ), 'mapping_id,channel_id,import_mapping_id')->result_array();
        if (count($roomMappingCheck) != 0) {
            foreach ($roomMappingCheck as $roomMap) {
                extract($roomMap);
                if ($channel_id == 11) {
                    $chaMapCheck = get_data(IM_RECO, array(
                        're_id' => $import_mapping_id
                    ), 'CODE')->row_array();
                    if (count($chaMapCheck) != 0) {
                        $chaReserCheckCount = $this->db->select('import_reserv_id')->from(REC_RESERV)->where(array(
                            'user_id' => current_user_type(),
                            'hotel_id' => hotel_id(),
                            'ROOMCODE' => $chaMapCheck['CODE']
                        ))->count_all_results();
                    }
                } else {
    ...
    {
        $ic_room_id      = $room_id;
        $hotel_details   = get_data(HOTEL, array(
            'hotel_id' => hotel_id()
        ))->row_array();
        $country_details = get_data(TBL_COUNTRY, array(
            'id' => $hotel_details['country']
        ))->row()->country_name;
...
METHOD:PUBLISH";
                ...
                if ($room_value->channel_id == 1) {
                    ...
                    if ($priceAmount != '' || $availability != '') {
                        ...
                        if ($priceAmount != '' && $room_value->update_rate == '1') {
                            ...
                            <SingleOcc>' . $priceAmount . '</SingleOcc>' . $mapping_fields . '
                            <Meals>' . $meal_plan . '</Meals>
                            <MinStay>' . $minimum_stay . '</MinStay>
                            <BlockStay>0</BlockStay>
                            <Guarantee>0</Guarantee>
    ...
    function channel_calendar_update($update_data, $chdata)
    {
        ...
        foreach ($update_data as $udata) {
            ...
            $room_mapping = get_data(MAP, array(
                'owner_id' => current_user_type(),
                'hotel_id' => hotel_id(),
                'channel_id' => $udata['individual_channel_id'],
                'property_id' => $udata['room_id'],
                'rate_id' => $rate,
                'guest_count' => $guest,
                'refun_type' => $refund,
                'enabled' => 'enabled'
                    ...
                    if ($room_value->channel_id == 1) {
                        ...
                        if ($price != '' || $ch_update_avail != '') {
                            ...
                            if ($price != '' && $room_value->update_rate == '1') {
                                ...
                                <StartDate>' . $up_sart_date . '</StartDate>
                                <EndDate>' . $up_sart_date . '</EndDate>
                                <SingleOcc>' . $price . '</SingleOcc>' . $mapping_fields . '
                                <Meals>' . $meal_plan . '</Meals>
                                <MinStay>' . $minimum_stay . '</MinStay>
                                <BlockStay>0</BlockStay>
                                <Guarantee>0</Guarantee>
                        ...
                        //echo "Response";print_r($airbnb_details);
                        ...
                        if (preg_match('/"access_token"\s*:\s*"(.*?)"/is', $resultFile, $match)) {
                            ...
                            if ($resultFile == '') {
                                ...
                                $this->store_error(current_user_type(), hotel_id(), $room_value->channel_id, "Could not get success message after update -  and DateRange : $start to $end.", 'Inline Edit No', date('m/d/Y h:i:s a', time()));
                            } elseif (preg_match('/"error_details"\s*:\s*"(.*?)"/is', $resultFile, $match)) {
                                
                                $this->store_error(current_user_type(), hotel_id(), $room_value->channel_id, $match[1] . "-DateRange : $start to $end.", 'Inline Edit No', date('m/d/Y h:i:s a', time()));
                            } elseif (preg_match('/error/is', $resultFile, $match)) {
                                
                                $this->store_error(current_user_type(), hotel_id(), $room_value->channel_id, "Could not get success message after update- and DateRange : $start to $end.", 'Inline Edit No', date('m/d/Y h:i:s a', time()));
                            }
                        } elseif (preg_match('/"error_message"\s*:\s*"(.*?)"/is', $resultFile, $match)) {
                            $this->store_error(current_user_type(), hotel_id(), $room_value->channel_id, $match[1], 'Inline Edit No', date('m/d/Y h:i:s a', time()));
                        } else {
                            $this->store_error(current_user_type(), hotel_id(), $room_value->channel_id, 'Access Token not generated.', 'Inline Edit No', date('m/d/Y h:i:s a', time()));
                        }
                    } else if ($room_value->channel_id == 5) {
                        $ch_details = get_data(CONNECT, array(
                            'user_id' => current_user_type(),
    ...
    function store_error($owner_id = '', $hotel_id = '', $channel_id, $message, $error_occurs, $datetime)
    {
        ...
        if ($error_occurs == 'Bulk Update' or $error_occurs == 'Bulk Update Save') {
            if (count($this->bulkdataArr) > 0) {
                $data['start_date']   = date('Y-m-d', strtotime($this->bulkdataArr['start_date']));
                $data['end_date']     = date('Y-m-d', strtotime($this->bulkdataArr['end_date']));
				if(array_key_exists('availability',$this->bulkdataArr)){
					$data['availability'] = $this->bulkdataArr['availability'];
				}
				if(array_key_exists('price',$this->bulkdataArr)){
					$data['price'] = $this->bulkdataArr['price'];
				}
				if(array_key_exists('minimum_stay',$this->bulkdataArr)){
					$data['minimum_stay'] = $this->bulkdataArr['minimum_stay'];
				}
				if(array_key_exists('cta',$this->bulkdataArr)){
					$data['cta'] = $this->bulkdataArr['cta'];
				}
				if(array_key_exists('ctd',$this->bulkdataArr)){
					$data['ctd'] = $this->bulkdataArr['ctd'];
				}
				if(array_key_exists('stop_sell',$this->bulkdataArr)){
					$data['stop_sell'] = $this->bulkdataArr['stop_sell'];
				}
				if(array_key_exists('open_room',$this->bulkdataArr)){
					$data['open_room'] = $this->bulkdataArr['open_room'];
				}
            }
        }
