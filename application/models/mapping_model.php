...
class Mapping_model extends CI_Model
{
      ...
      function SaveConvertion()
    {
    	...
    	$import_mapping_ids = $this->input->post('ids');
    	$ValConvertion = $this->input->post('Conversion');
    	$channel_id=  $this->input->post('channel_id');
    	$hotel_id = hotel_id();
    	$this->db->query("update roommapping  set rate_conversion = $ValConvertion where import_mapping_id in ($import_mapping_ids ) and channel_id= $channel_id and hotel_id =$hotel_id ");
...
*/
		...
		if($channel_id==1)
		{
			...
			$exp_occupancy	=	$this->input->post('occupancy_two');
		}
		...
		if (isset($exp_occupancy))
		{
			$exp_occupancyR = $exp_occupancy;
		}
		else
		{
			$exp_occupancyR = "";
		...
		$data=array(
						...
						'extra_adult'=>$this->input->post('extra_adults'),    
						'extra_child'=>$this->input->post('extra_children'),
						...
						'explevel' => $levelmapp,
						'exp_occupancy' =>$exp_occupancyR,
						'ChargeType' =>$this->input->post('ChargeTypeID'),
						'Adults' =>$this->input->post('Adult1'),
						'Children' =>$this->input->post('Children1'),
						'Infants' =>$this->input->post('Infants1'),
	...
	function connect_channel($channel_id)
	{
		...
		else
		{
			...
			$data = array('user_id'=>current_user_type(),'hotel_id'=>hotel_id(),'channel_id'=>$channel_id,'status'=>$status,'rate_multiplier'=>$rate_conversion,'user_name'=>$user_name,'reservation_email'=>$email_address,'other_id' => $other_id,'user_password'=>$user_password,'hotel_channel_id'=>$hotel_channel_id,'web_id'=>$web_id,'test_url' => $test_url, 'live_url'=>$live_url,'mode'=>$mode,'xml_type'=>$xml_type);
	...
	{	
		...
		if($channel_id=='11')
		{
			...
			$this->db->select('R.IDROOM roomid, R.CODE , R.re_id import_mapping_id, r.IDRATECODE rateid');
			if($clean!='')
			{
		...
		elseif($channel_id=='19')
		{
			...
			$clean = $this->cleanArray($import);
			/* ----- Get Sub Rooms ------ */
			$this->db->select('E.map_id import_mapping_id, E.roomtype_name, E.roomtype_id roomid, E.name RoomName ,E.rate_type_id rateid');
			if($clean!='')
			{
				$this->db->where_not_in('E.map_id',$import);
			}
			$this->db->where('rate_type_id !=', '');
			$this->db->where(array('channel'=>$channel_id,'user_id'=>$owner_id,'hotel_id'=>hotel_id()));
			$result = $this->db->get('import_mapping_AGODA as E');
		...
		elseif($channel_id=='1')
		{
			...
			/* ----- Get Sub Rooms ------ */
			$this->db->select('E.map_id import_mapping_id, E.roomtype_name, E.roomtype_id roomid, E.code, roomtype_name RoomName, E.distributionModel, E.rateAcquisitionType, rate_type_id rateid');
			if($clean!='')
			{
				$this->db->where_not_in('E.map_id',$import);
			}
		...
		elseif($channel_id=='36')
		{
			...
			/* ----- Get Sub Rooms ------ */
			$this->db->select('E.import_mapping_id, E.nameRoomType, E.codeRoomType roomid, E.rate_name, E.RateTypeCode rateid');
			if($clean!='')
			{
				$this->db->where_not_in('E.import_mapping_id',$import);
			}
			//$this->db->where_not_in('E.distributionModel',"ExpediaCollect");
			$this->db->where(array('user_id'=>$owner_id,'hotel_id'=>hotel_id()));
			$this->db->where('E.RateTypeCode !=', '0');
			$result = $this->db->get('import_mapping_DESPEGAR'.' as E');
			/* ----- End Of Get Sub Rooms ------ */
			if($result!='')
			{
				return $result->result_array();
		...
		elseif($channel_id=='2')
		{
			...
			$this->db->select('B.import_mapping_id, B.room_name as RoomName, B.policy_id, B.rate_name,B.B_rate_id rateid,B.B_room_id roomid');
			if($clean!='')
			{
		...
		elseif($channel_id=='8')
		{
			...
			$this->db->select('G.GTA_id import_mapping_id, G.RoomType roomid, G.Description RoomName,G.rateplan_code,G.contract_type,G.ID,G.RateBasis,G.MaxOccupancy, rateplan_id rateid');
			if($clean!='')
			{
		...
		elseif($channel_id=='5')
		{
			...
			/* ----- Get Sub Rooms ------ */
			$this->db->select('H.map_id import_mapping_id, H.contract_name, H.sequence, H.roomname, H.roomtype roomid, H.characterstics, 0rateid');
			if($clean!='')
			{
		...
		elseif ($channel_id == '14') {
		    ...
		    /* ----- Get Sub Rooms ------ */
		    $this->db->select('A.import_mapping_id, A.codeRoomType roomid, A.nameRoomType RoomName, codeRate rateid');
		    if ($clean != '') {
			$this->db->where_not_in('A.import_mapping_id', $import);
		    }
		    $this->db->where(array(
		    ...
		    $result = $this->db->get('import_mapping_WBEDS' . ' as A');
		    /* ----- End Of Get Sub Rooms ------ */
		    if ($result != '') {
			return $result->result_array();
		    } else {
		...
		elseif ($channel_id == '40' || $channel_id == '41'|| $channel_id == '42' ) {
		    ...
		    /* ----- Get Sub Rooms ------ */
		    $this->db->select('A.map_id import_mapping_id, A.roomcode roomid, A.roomname RoomName, A.mealplan  rateid');
		    if ($clean != '') {
			$this->db->where_not_in('A.map_id', $import);
		    }
		    $this->db->where(array(
			'hotel_id' => hotel_id(),
			'channel_id'=>$channel_id
		    ));
		    $result = $this->db->get('import_mapping_HOTUSAGROUP' . ' as A');
		    /* ----- End Of Get Sub Rooms ------ */
		    if ($result != '') {
			return $result->result_array();
		    } else {
	...
	{	
		...
		elseif($channel_id=='1')
		{
			...
			elseif($channel_id=='36')
		{
			$count = $this->db->select('E.import_mapping_id')->from('import_mapping_DESPEGAR')->where(array('user_id'=>current_user_type(),'hotel_id'=>hotel_id(),'hotelcode'=>$ch_details->hotel_channel_id))->count_all_results();
			return $count;
		}
		elseif($channel_id=='19')
		{
			$count = $this->db->select('map_id')->from('import_mapping_AGODA')->where(array('user_id'=>current_user_type(),'hotel_id'=>hotel_id(),'hotel_channel_id'=>$ch_details->hotel_channel_id))->count_all_results();
			return $count;
		}
		elseif($channel_id=='2')
		{
		...
		elseif ($channel_id == '9') {
		    $count = $this->db->select('import_mapping_id')->from('import_mapping_AIRBNB')->where(array(
			'user_id' => current_user_type(),
			'hotel_id' => hotel_id()
		    ))->count_all_results();
		    return $count;
	...
	function get_all_mapped_rooms($channel_id)
	{
		...
		else if($channel_id==5)
		{
		 ...
		 $this->db->join('manage_property as C', 'R.property_id=C.property_id');
		$this->db->where(array('R.hotel_id'=>hotel_id(),'R.channel_id'=>$channel_id));
	...
	function get_all_mapped_rooms_count($channel_id)
	{
		...
		{
			$this->db->join('import_mapping_DESPEGAR as E','R.import_mapping_id=E.import_mapping_id');
			//$this->db->join(CONNECT.' as C','E.hotelcode=C.hotel_channel_id  ');
		}
		else if($channel_id==11)
		{
